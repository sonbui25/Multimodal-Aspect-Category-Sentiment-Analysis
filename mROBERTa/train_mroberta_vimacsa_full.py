...
# ==============================================================================
# 2. DATASET (mRoBERTa Standard Format)
# ==============================================================================

class mRoBERTaDataset(Dataset):
    """
    Dataset class cho mRoBERTa theo paper Yu & Jiang (2019).
    Input format: [CLS] Aspect [SEP] Sentence [SEP] (đối với RoBERTa là <s> Aspect </s></s> Sentence </s>)
    """
    def __init__(self, data, tokenizer, img_folder, roi_df, dict_image_aspect, dict_roi_aspect, num_img, num_roi, max_len=170):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        # --- Image Loading (Standard ResNet Features) ---
        ...
        # --- Text Processing (Single Stream: Aspect + Text) ---
        ...
        for ix in range(len(self.ASPECT)):
            ...
            # Construct Input: <s> Aspect </s></s> Text </s>
            # We use tokenizer functionality to handle the pair construction automatically
            combined_input = self.tokenizer(
                asp.lower(), 
            ...
            out_input_ids.append(combined_input['input_ids'].squeeze(0))
            out_attention_mask.append(combined_input['attention_mask'].squeeze(0))
            out_labels.append(self.pola_to_num[list_polar[idx_asp_in_list_asp]])

        return t_img_features, torch.tensor(roi_img_features), \
               torch.stack(out_input_ids), torch.stack(out_attention_mask), \
               torch.tensor(out_labels), text

# ==============================================================================
# 3. MODEL (mRoBERTa Architecture)
# ==============================================================================
...
class mRoBERTa(nn.Module):
    """
    Multimodal RoBERTa Architecture (adapted from mBERT - Yu & Jiang, 2019):
    1. Text Encoder (RoBERTa) -> Process [Aspect + Text]
    2. Visual Encoder (ResNet) -> Process Images
    3. Cross-Modal Attention (TIM) -> Text attends to Image to get target-aware visual features.
    4. Multimodal Encoder -> Self-Attention layers on top to fuse modalities.
    """
    def __init__(self, pretrained_path, num_labels=4):
        ...
        # Projection for Visual Features (2048 -> Hidden Size)
        self.vis_projection = nn.Linear(2048, self.hidden_size)
        self.roi_projection = nn.Linear(2048, self.hidden_size)
        
        # 1. Cross-Modal Attention (Text Querying Image)
        # Using Multihead Attention to implement Target-Image Matching
        ...
        self.norm_cross = nn.LayerNorm(self.hidden_size)

        # 2. Multimodal Encoder (Stack of Transformer Layers)
        # Paper suggests stacking layers on top of BERT output
        mm_layer = nn.TransformerEncoderLayer(
        ...
        self.mm_encoder = nn.TransformerEncoder(mm_layer, num_layers=3) # Stack 3 layers (Tunable)
        
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(self.hidden_size, num_labels)
        
        self.apply_custom_init(self.classifier)
        self.apply_custom_init(self.vis_projection)
        self.apply_custom_init(self.roi_projection)
    ...
    def forward(self, input_ids, attention_mask, visual_embeds_att, roi_embeds_att):
        # --- 1. Text Encoding ---
        # input_ids: [Batch, Seq_Len] (<s> Aspect </s></s> Text </s>)
        text_outputs = self.roberta(input_ids=input_ids, attention_mask=attention_mask)
        text_feats = text_outputs.last_hidden_state # [Batch, Seq, Hidden]
        ...
        # Concatenate all visual cues (Patches + ROIs)
        visual_feats = torch.cat([vis_proj, roi_proj], dim=1) # [Batch, Total_Vis, Hidden]
        
        # --- 3. Cross-Modal Attention (Target-Image Matching) ---
        # The text sequence acts as Query to find relevant regions in Visual features (Key/Value)
        # attn_output: [Batch, Seq, Hidden] - Aligned visual info for each text token
        attn_output, _ = self.cross_attention(query=text_feats, key=visual_feats, value=visual_feats)
        
        # Residual connection + Norm (Standard Transformer Block style)
        # We fuse the attended visual features back into the text stream
        fused_feats = self.norm_cross(text_feats + attn_output)
        
        # --- 4. Multimodal Encoder ---
        # Pass the visually-enriched text sequence through additional Transformer layers
        # Key padding mask is needed to ignore pad tokens from text
        # PyTorch expects True for padded positions
        src_key_padding_mask = (attention_mask == 0)
        
        mm_output = self.mm_encoder(fused_feats, src_key_padding_mask=src_key_padding_mask)
        
        # --- 5. Classification ---
        # Use the [CLS] token (index 0) from the final multimodal representation
        cls_output = mm_output[:, 0, :]
...
def main():
    ...
    logging.basicConfig(format='%(asctime)s - %(levelname)s - %(name)s - %(message)s', datefmt='%m/%d/%Y %H:%M:%S', level=logging.INFO, handlers=[logging.FileHandler(f'{args.output_dir}/training_mroberta.log'), logging.StreamHandler()])
    ...
    if args.do_train:
        ...
        train_dataset = mRoBERTaDataset(train_data, tokenizer, args.image_dir, roi_df, dict_image_aspect, dict_roi_aspect, args.num_imgs, args.num_rois)
        dev_dataset = mRoBERTaDataset(dev_data, tokenizer, args.image_dir, roi_df, dict_image_aspect, dict_roi_aspect, args.num_imgs, args.num_rois)

    # --- Model ---
    model = mRoBERTa(pretrained_path=args.pretrained_hf_model, num_labels=args.num_polarity)
    model.to(device)
        ...
        for epoch in range(start_epoch, int(args.num_train_epochs)):
            ...
            with tqdm(train_loader, desc=f"Epoch {epoch}", dynamic_ncols=True) as tepoch:
                for step, batch in enumerate(tepoch):
                    ...
                    t_img, roi_img, input_ids, attention_mask, labels, _ = batch
                    ...
                    with autocast(enabled=args.fp16):
                        ...
                        for id_asp in range(len(ASPECT)):
                            logits = model(input_ids=input_ids[:,id_asp,:], attention_mask=attention_mask[:,id_asp,:],
            ...
            if args.do_eval:
                ...
                if avg_f1 > max_f1:
                    max_f1 = avg_f1
                    save_model(f'{args.output_dir}/mroberta_best.pth', model, optimizer, scheduler, epoch, max_f1, scaler)
    ...
    if args.do_eval:
        ...
        test_dataset = mRoBERTaDataset(test_data, tokenizer, args.image_dir, roi_df, dict_image_aspect, dict_roi_aspect, args.num_imgs, args.num_rois)
        test_loader = DataLoader(test_dataset, sampler=SequentialSampler(test_dataset), batch_size=args.eval_batch_size)

        best_path = f'{args.output_dir}/mroberta_best.pth'
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc="Testing"):
                ...
                t_img, roi_img, input_ids, attention_mask, labels = batch_tensors
                roi_img = roi_img.float()
        ...
        # Save Metrics
        with open(os.path.join(args.output_dir, "test_results_mroberta.txt"), "w") as writer:
